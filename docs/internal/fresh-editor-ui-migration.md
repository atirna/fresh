...
**Second step, done: the flip itself.** `Splits::interiors` carries the
per-pane `Interior`, `live_interior` returns that subtree in place of
`content_surface`, and the gate is `widgets::covered` — so a `WindowEmbed`
panel keeps its painter whole, by G's rule, exactly as the floating panel's
does. The thing this step had to decide that C.5b did not was answered by
