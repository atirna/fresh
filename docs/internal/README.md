...
| [welcome-screen-design.md](welcome-screen-design.md) | Design for an interactive **welcome screen** replacing the `[No Name]` scratch buffer and the blank-pane hint as the default empty-workspace surface — at launch with nothing to restore and after the last buffer closes. Argues it should be a render mode for the existing `synthetic_placeholder` pane rather than a buffer or a plugin, and specifies the two-column layout (editing and agent verbs on the left; recent projects and orchestrator sessions as one badged list on the right), keyboard/mouse interaction, the `editor.empty_workspace_screen` setting and its migration from `auto_create_empty_buffer_on_last_buffer_close`, fold breakpoints with ASCII wireframes, and a phased build order. Entirely planned. |
