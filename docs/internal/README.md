...
| [sidebar-sections-design.md](sidebar-sections-design.md) | Turning the file-explorer sidebar from a single-occupancy column into a column of stacked, individually collapsible sections sharing one border row per boundary (sinelaw/fresh#3045). Audits the four panel placements and the three separate divider-drag implementations Fresh already has, argues the request is the missing generalization rather than a duplicate, and specifies the section model, sizing, focus, persistence defaults and plugin op. Sequenced deliberately *after* the UI migration's explorer stage, because every mechanism it needs is one the migration deletes or provides. Entirely planned. |
